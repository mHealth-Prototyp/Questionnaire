...
export default boot(async ({router}) => {
  router.beforeEach((to) => {
    ...
    (i18n.global.locale as unknown as Ref<string>).value = store.userSettings.language;
    ...
    const splitFullPath = to.fullPath.split('/');
    if (splitFullPath.length > 1 && splitFullPath[1] === 's') {
      if (store.userQuestionnaire === null) return {path: '/'};
    }
  });
});
